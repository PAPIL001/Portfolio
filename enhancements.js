...
function initCursorTrail() {
  ...
  const trails = [];
  const BAT_PATH =
    "M 600,650 C 540,450 430,400 320,420 C 290,300 190,220 40,180 C 190,130 380,150 500,250 L 570,120 L 570,200 L 630,200 L 630,120 L 700,250 C 820,150 1010,130 1160,180 C 1010,220 910,300 880,420 C 770,400 660,450 600,650 Z";
...
function runHeroGSAP() {
  if (typeof gsap === "undefined") {
    document
      .querySelectorAll("#hero [data-animate]")
...
function initBatTerminal() {
  ...
  async function printWelcome() {
    ...
    await printLine(
      "&nbsp; BAT-COMPUTER INTERFACE — AUTHORIZED ACCESS ONLY &nbsp;",
      "yellow",
      120,
    );
    await printLine(
    ...
    await printLine(
      '&gt; Type <span class="yellow">help</span> to see available commands.',
      "dim",
      340,
    );
    await printLine("", "", 380);
  ...
  const COMMANDS = {
    ...
    whoami: () => [
      ...
      {
        t: '&gt; STATUS: <span class="yellow">AVAILABLE FOR HIRE</span>',
        c: "",
      },
    ...
    projects: () => [
      ...
      {
        t: "  #001 — Music Streaming App       [Node.js / MongoDB / Express]",
        c: "",
      },
      {
        t: "  #002 — EntityGraph AI            [Python / OpenAI / Graph]",
        c: "",
      },
      {
        t: "  #003 — Healthcare Web Platform   [Django / React / Azure]",
        c: "",
      },
      { t: "  #004 — Data Pipeline Automation  [Python / GCP / ML]", c: "" },
      {
        t: "  #005 — Batman Portfolio          [HTML / CSS / JS / GSAP]",
        c: "",
      },
      {
        t: "  #006 — REST API Microservices    [Node.js / Express / REST]",
        c: "",
      },
    ...
    exit: () => {
      ...
      return [{ t: "&gt; Closing secure channel...", c: "dim" }];
    },
  };
  ...
  if (input) {
    input.addEventListener("keydown", async (e) => {
      ...
      if (!cmd) return;
      await printLine(
        `<span class="yellow">BATMAN@GOTHAM:~$</span> ${cmd}`,
        "white",
      );
      ...
      if (handler) {
        ...
        await printLine(
          `&gt; Command not found: "${cmd}". Type <span class="yellow">help</span>.`,
          "red",
        );
      }
      await printLine("", "", 0);
...
function initDecryptResume() {
  ...
  const RESUME_URL =
    "https://drive.google.com/file/d/1oGGUNyJygLSxTadOQGtkW78tr0D0XQqY/view?usp=sharing";
